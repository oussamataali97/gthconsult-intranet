<template>
  <div class="creationClient">
    ...
    <div class="formCreation">
      ...
      <div>
        <input type="submit" value="Modifié un Fournisseur" @click="update()">
      </div>
...
export default {
    ...
    update() {
      Service.updateFournisseur(this.fournisseur, this.fournisseurId)
        .then((response) => {
            ...
            this.msgFournisseur = "Veuillez patienter quelques secondes pour modfier";
            if(response) {
                setTimeout(() => {
  ...
  created() {
    ...
    Service.selectFournisseur(this.fournisseurId)
      .then((response) => {
            if(response) {
                this.fournisseur.raisonSocial = response.data.fournisseur.raisonSocial;
                this.fournisseur.formJuridique = response.data.fournisseur.formJuridique;
                this.fournisseur.ville = response.data.fournisseur.ville;
                this.fournisseur.codePostal = response.data.fournisseur.codePostal;
                this.fournisseur.pays = response.data.fournisseur.pays;
                this.fournisseur.email = response.data.fournisseur.email;
                this.fournisseur.ice = response.data.fournisseur.ice;
                this.fournisseur.telephone = response.data.fournisseur.telephone;
                this.fournisseur.id = response.data.fournisseur._id;
                this.fournisseur.activite = response.data.fournisseur.activite;
            }
      })
      .catch((error) => {
          this.error = error.message;
...
#app > div > div > div.menu-content > div.content > div > div > div > div:last-child > input[type=submit] {
    background-color: red;
    color: white;
    border: 0;
    margin-top: 50px;
...
#app > div > div > div.menu-content > div.content > div > div > div > div > input[type=submit] {
