...
export default {
  ...
  //Delete Production -> delete Intervention
  deleteIntervention(affaireId, index) {
    Service.DeleteIntervention(affaireId)
    .then((result) => {
        if(result) {
    ...
    // read all Affaires LOCAL
      ...
      .then((result) => {
        ...
        result.data.result.forEach((element) => {
          ...
          var lastSlash = element.numeroAffaire.lastIndexOf("|");
          var nomSociete = element.numeroAffaire.substr(lastSlash + 1)
          this.arrayNumAffairesLocal.push({nomSociete, importRapport : element.importRapport, numeroAffaire : element.numeroAffaire});

        });

        // get all rapport cloud
        // Service.checkRapports()
        // .then((response) => {
        //   response.rapports.forEach((rapport) => {
        //     this.arrayNumAffairesCloud.push(rapport.referenceRapport);
        //   })
        // })
        // .catch((err) => {
        //   console.log(err)
